...
class BookShellComponentTest {
  ...
  private static final String MISSING_BOOK_ID_VALUE = "4";
  ...
  void init() {
    ...
    Mockito
        .doAnswer(invocation -> {
          ...
          return String.format("Book id: %s%stitle: %s%sisbn: %s%swritten by: %s in genre: %s%s",
              book.getId(),
              System.lineSeparator(),
              book.getTitle(),
              System.lineSeparator(),
  ...
  void shouldBeInsertNewBook() {
    ...
    Mockito
        ...
        .when(bookService)
        .saveOrUpdate(exceptedBookFullDto);
    String successCreatedBookMessage = (String) shell.evaluate(() -> String.format("create-book --title %s --isbn %s --author-id %s --genre-id %s",
  ...
  void shouldBeThrowDuplicateKeyExceptionByNaturalKey() {
    ...
    exceptedBook.setGenre(exceptedGenre);
    exceptedBook.setId(null);
    exceptedBook.setTitle("Girl_from_the_Earth");
    BookFullDto exceptedBookFullDto = bookMapper.toFullDto(exceptedBook);
    Mockito
        ...
        .when(bookService)
        .saveOrUpdate(exceptedBookFullDto);
    String errorCreatedBookMessage = (String) shell.evaluate(() -> String.format("create-book --title %s --isbn %s --author-id %s --genre-id %s",
        exceptedBook.getTitle(),
        exceptedBook.getIsbn(),
        exceptedBook
            .getAuthor()
    ...
    assertThat(errorCreatedBookMessage)
        .isNotNull()
        .isEqualTo("Can't create book Cause unique key violation!");
  }
  //endregion
  ...
  void shouldBeGetBookById() {
    ...
    String successAllBookReadCommandOutput = (String) shell.evaluate(() -> String.format("read-book --id %s",
  ...
  void shouldBeThrowPersistenceExceptionInGetAllBook() {
    Mockito
        .doThrow(new LibraryGeneralException("Can't read book",
            new MongoException("Book not found with id " + MISSING_BOOK_ID_VALUE)))
        .when(bookService)
        .findById(MISSING_BOOK_ID_VALUE);
    String errorReadBookMessage = (String) shell.evaluate(() -> String.format("read-book --id %s",
  ...
  void shouldBeUpdateInsertedBookTitle() {
    ...
    String successUpdateBookMessage = (String) shell.evaluate(() -> String.format("update-book --id %s --title %s",
        EXISTING_BOOK_ID_VALUE,
  ...
  @Test
  void shouldBeMongoExceptionInUpdateBook() {
    ...
    BookFullDto emptyNotExistingBookFullDto = bookMapper.toFullDto(emptyNotExistingBook);
    Mockito
        .doThrow(new LibraryGeneralException("Can't update book",
            new MongoException("Book not found with id " + MISSING_BOOK_ID_VALUE)))
        .when(bookService)
        .saveOrUpdate(emptyNotExistingBookFullDto);
    String errorUpdateBookMessage = (String) shell.evaluate(() -> String.format("update-book --id %s",
        MISSING_BOOK_ID_VALUE));
  ...
  void shouldBeDeleteInsertedBook() {
    ...
    String successDeletedBookMessage = (String) shell.evaluate(() -> String.format("delete-book --id %s",
        EXISTING_BOOK_ID_VALUE));
  ...
  @Test
  void shouldBeMongoExceptionInDeleteBookById() {
    Mockito
        .doThrow(new LibraryGeneralException("Can't delete book",
            new MongoException("Book not found with id " + MISSING_BOOK_ID_VALUE)))
        .when(bookService)
        .deleteById(MISSING_BOOK_ID_VALUE);
    String deletedBookErrorMessage = (String) shell.evaluate(() -> String.format("delete-book --id %s",
        MISSING_BOOK_ID_VALUE));
